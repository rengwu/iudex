...
|-----------|---------|---------|
| **`Button`** | Flat square action button; color = state | `variant` `primary`(amber) · `secondary`(gray) · `review`(violet) · `danger`(red) · `success`(green) · `info`(blue) · `quiet`(transparent, light surface) · `quietDark`(transparent, for dark surfaces e.g. a selected card); `size` `sm`(20px) / `md`(22px) / `lg`(30px). Interaction states (hover/active/focus-visible): [`button-interaction-states.md`](./button-interaction-states.md) |
| **`IconButton`** | The one bare glyph button (close ✕ / dismiss / kill) | `tone` `neutral` / `danger`(hover→red); `size` `sm`(12px) / `md`(14px); glyph via `children` (defaults ✕). Inherits surface text color at reduced opacity → surface-agnostic. Labeled nav (e.g. "← Back") stays a text link, not this |
...
| **`TabSwitcher`** | Segmented control, 3 variants | `tabs`, `value`, `onChange`; `variant` `pill`(default, gray value-picker) · `mode`/`modeDark`(amber active + radiogroup, a persisted setting — light track for light panels, dark track for the Sidebar rail) · `tabs`/`tabsDark`(top-accent bar over a body — `tabsDark` is the dark, mint-accented terminal chrome); `stretch` fills the container; `onClose` makes tabs closable (per-tab ✕ + middle-click, e.g. Terminal sessions); `ariaLabel`/`fontSize`. Inset `:focus-visible` ring in `currentColor` (surface-agnostic) |
